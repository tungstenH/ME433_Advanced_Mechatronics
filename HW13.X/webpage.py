...
def webpage():
    ...
    <h1>Control the timer with this button</h1><br>
    <p><button class="timer_button" name="TIMER" value="ON" type="submit">TIMER BUTTON</button></a></p>
    <p class="dotted">The current timer is
    <span style="color: deeppink;"><span id="timing">{timing}</p><br>
    </body>
    <script>
    // LED buttons handler
    $(".button").click((el) => {{ $.post("/led", {{[el.target.name]: el.target.value}}) }});
    // TIMER buttos handler
    $(".timer_button").click((el) => {{
      $.get("/timing", (data) => {{
            $("#timing").text(data);
        }});)
    }});
...
# change led state
prev_time = 0
@server.route("/timing", method=HTTPMethod.POST)
def timing(request: HTTPRequest):
    print("Timer button request")
    curr_time = time.time()
    time_button = curr_time - prev_time
    prev_time = curr_time
    # Acknowledge
    with HTTPResponse(request, content_type=MIMEType.TYPE_HTML) as response:
        response.send(time_button)
